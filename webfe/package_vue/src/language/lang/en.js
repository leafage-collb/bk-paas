...
export default {
  ...
  '启用 IP 限制后，仅白名单中的 IP 可访问应用主模块，预发布和生产环境同时生效。': 'After enabling IP restrictions, only IPs in the whitelist can access the main module of the application, effective in both pre-release and production environments.',
  '功能开启后，预发布和生产环境的 IP 限制将立即生效，仅限白名单内 IP 可访问，且该限制仅适用于主模块。': 'Once the feature is enabled, IP restrictions in pre-release and production environments will take effect immediately, allowing access only for IPs within the whitelist, and this restriction applies solely to the main module.',
  '功能停用后，主模块的预发布和生产环境的 IP 限制将立即取消，所有 IP 均可访问。': 'Once the feature is disabled, the IP restrictions for the main module in pre-release and production environments will be lifted immediately, allowing access for all IPs.',
  添加白名单: 'Add whitelist',
  ...
  '功能开启后，预发布和生产环境的用户限制将立即生效，仅限白名单用户访问，且该限制仅适用于主模块。': 'Once the feature is enabled, user restrictions in pre-release and production environments will take effect immediately, limiting access to whitelisted users only, and this restriction applies only to the main module.',
  '功能停用后，主模块的预发布和生产环境的用户限制将立即取消，所有用户均可访问。': 'Once the feature is disabled, user restrictions for the main module in pre-release and production environments will be lifted immediately, allowing access to all users.',
  ...
  '启用用户限制后，仅白名单用户可访问应用主模块，预发布和生产环境同时生效。': 'Once user restrictions are enabled, only whitelisted users can access the applications main module, effective in both pre-release and production environments.',
  ...
  '应用 ID': 'APP ID',
  '格式不正确，只能包含：3-20 字符的小写字母、数字、连字符(-)，以小写字母开头': 'Incorrect format, 3-20 Characters with lowercase letters, numbers, hyphens (-), beginning with lowercase letters',
  ...
  '由小写字母组成，长度小于 20 个字符': 'Consists of lowercase letters and is less than 20 characters long',
  ...
  '只允许小写字母、数字、连字符(-)和下划线(_)，以字母开头，长度3-20个字符':
    'Only lowercase letters, numbers, hyphens (-), and underscores (_) are allowed, starting with a letter, length 3-20 characters',
  ...
  '将自动创建该私有仓库并完成模板代码初始化{t}，当前用户默认为仓库管理员。': 'The private repository will be automatically created and the template code will be initialized{t}, with the current user set as the default repository administrator',
  '（默认使用 Dockerfile 构建，创建成功后可修改）': '(Dockerfile build by default, can be modified after creation)',
  ...
  'S-mart 打包工具': 'S-mart Packaging Tool',
  '构建 S-mart 包': 'Build S-mart Package',
  '请输入分支 / 包名': 'Please enter branch / package name',
  打包历史: 'Packaging History',
  执行详情: 'Execution Details',
  构建: 'Build',
  源码来源: 'Source Origin',
  执行成功: 'Execution Successful',
  上传源码包: 'Upload Source Code Package',
  从代码仓库克隆: 'Clone from Repository',
  多模块应用: 'Multi-module Application',
  单模块应用: 'Single-module Application',
  'S-mart 应用特性说明': 'S-mart Application Feature Explanation',
