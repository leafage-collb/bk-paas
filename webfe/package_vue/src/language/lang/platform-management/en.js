...
export default {
  ...
  '由 3-32 位字母、数字、下划线(_) 字符组成，以字母开头，字母或数字结尾': 'Consists of 3-32 letters, numbers, and underscores (_), starts with a letter, and ends with a letter or number',
  ...
  '此处设置的资源配额优先级高于应用描述文件（app_desc.yaml），重新部署后生效。仅适用于 S-mart 应用与 AI 插件应用。': 'The resource quotas set here take precedence over the application description file (app_desc.yaml) and will take effect after redeployment. Only applicable to S-mart applications and AI plugin applications.',
  'Requests 不能大于 Limits': 'Requests cannot exceed Limits',
  显示配置: 'Show configuration',
  隐藏配置: 'Hide configuration',
  云原生应用资源配额: 'App Resource Quota',
  资源方案管理: 'Resource Plan Management',
  '是否删除该项目方案？': 'Are you sure you want to delete this project scheme?',
  '删除后，应用将不能再绑定该方案，请谨慎操作': 'Once deleted, the application will not be able to bind to this scheme again. Please act cautiously.',
  '停用后不影响已绑定的实例，但不能再绑定到新的应用': 'Deactivation does not affect already bound instances, but they cannot be bound to new applications.',
  '方案名称将以由 CPU/内存 Limit 自动生成，也可以自定义输入': 'Auto-generated by CPU/Memory Limit or custom input.',
  '资源方案添加后，应用将能够在“模块配置 - 进程配置”的资源配额方案中进行选择。': 'After adding the resource plan, the application will be able to select it in the "Module Configuration - Process Configuration" resource quota options.',
