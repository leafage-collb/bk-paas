...
export default {
  ...
  '由 3-32 位字母、数字、下划线(_) 字符组成，以字母开头，字母或数字结尾': '由 3-32 位字母、数字、下划线(_) 字符组成，以字母开头，字母或数字结尾',
  平台管理员: '平台管理员',
  添加方式: '添加方式',
  ...
  '描述（中）': '描述（中）',
  '描述（英）': '描述（英）',
  '只能由字符 [a-zA-Z0-9-_] 组成': '只能由字符 [a-zA-Z0-9-_] 组成',
  '用户在创建应用/模块时仅能使用未隐藏的模板，但现有应用仍可下载已隐藏的模板。': '用户在创建应用/模块时仅能使用未隐藏的模板，但现有应用仍可下载已隐藏的模板。',
  确认删除模版配置: '确认删除模版配置',
  '删除后，创建应用时将不能再使用该模版。': '删除后，创建应用时将不能再使用该模版。',
  请输入要删除的模版配置的模版名称: '请输入要删除的模版配置的模版名称',
  请输入对象存储中的包路径: '请输入对象存储中的包路径',
  请输入代码仓库地址: '请输入代码仓库地址',
  '请输入代码所在目录，不填则为根目录': '请输入代码所在目录，不填则为根目录',
  克隆: '克隆',
  克隆成功: '克隆成功',
  '当前用户无可用的应用集群，无法创建应用；请联系平台管理员添加集群或调整集群分配策略。': '当前用户无可用的应用集群，无法创建应用；请联系平台管理员添加集群或调整集群分配策略。',
  删除时间: '删除时间',
  彻底删除: '彻底删除',
  管理已删除应用: '管理已删除应用',
  确认彻底删除应用: '确认彻底删除应用',
  '删除应用时，应用只是被标记为已删除，同名应用无法被重新创建。如需彻底删除应用数据，请使用 “彻底删除” 功能。': '删除应用时，应用只是被标记为已删除，同名应用无法被重新创建。如需彻底删除应用数据，请使用 “彻底删除” 功能。',
  实例凭证: '实例凭证',
  配置示例: '配置示例',
  字段: '字段',
  必填: '必填',
  示例: '示例',
  '当前模块未部署，无法获取进程列表': '当前模块未部署，无法获取进程列表',
  '此处设置的资源配额优先级高于应用描述文件（app_desc.yaml），重新部署后生效。仅适用于 S-mart 应用与 AI 插件应用。': '此处设置的资源配额优先级高于应用描述文件（app_desc.yaml），重新部署后生效。仅适用于 S-mart 应用与 AI 插件应用。',
  'Requests 不能大于 Limits': 'Requests 不能大于 Limits',
  显示配置: '显示配置',
  隐藏配置: '隐藏配置',
  云原生应用资源配额: '云原生应用资源配额',
  资源方案管理: '资源方案管理',
  '是否删除该项目方案？': '是否删除该项目方案？',
  '删除后，应用将不能再绑定该方案，请谨慎操作': '删除后，应用将不能再绑定该方案，请谨慎操作',
  '停用后不影响已绑定的实例，但不能再绑定到新的应用': '停用后不影响已绑定的实例，但不能再绑定到新的应用',
  '方案名称将以由 CPU/内存 Limit 自动生成，也可以自定义输入': '方案名称将以由 CPU/内存 Limit 自动生成，也可以自定义输入',
  '资源方案添加后，应用将能够在“模块配置 - 进程配置”的资源配额方案中进行选择。': '资源方案添加后，应用将能够在“模块配置 - 进程配置”的资源配额方案中进行选择。',
  编辑资源配额: '编辑资源配额',
  源码摘要: '源码摘要',
  '支持开发者中心 1.5.x 及以上版本': '支持开发者中心 1.5.x 及以上版本',
  '支持开发者中心 1.7.x 及以上版本，构建更快、包更小，适合多模块共享代码的场景': '支持开发者中心 1.7.x 及以上版本，构建更快、包更小，适合多模块共享代码的场景',
  配置项: '配置项',
  预览环境变量: '预览环境变量',
  按顺序分配: '按顺序分配',
  配置项将以环境变量方式注入至应用运行时环境: '配置项将以环境变量方式注入至应用运行时环境',
  '应用申请增强服务实例时，按 FIFO（先进先出）顺序分配。': '应用申请增强服务实例时，按 FIFO（先进先出）顺序分配。',
  'TLS 相关的配置（证书和密钥）以文件形式存储在容器内; 环境变量 {id}_CERT、{id}_KEY、{id}_CA 分别指向对应文件的路径': 'TLS 相关的配置（证书和密钥）以文件形式存储在容器内; 环境变量 {id}_CERT、{id}_KEY、{id}_CA 分别指向对应文件的路径',
  ...
  已使用: '已使用',
  未使用: '未使用',
  '方案（{name}）正被以下 <i>{count}</i> 个应用、模块使用：': '方案（{name}）正被以下 <i>{count}</i> 个应用、模块使用：'
};
