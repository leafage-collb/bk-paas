<template>
  ...
  >
    ...
    >
      <p class="tips">
        {{ $t('如需接入统计，请在页面尾部的') }} &lt;/body&gt; {{ $t('标签前，增加以下代码：') }}
      </p>
      <p class="tips warn">
        {{ $t('注意：所有使用蓝鲸 Python(Django) 框架的应用默认已嵌入该脚本，无需重复添加。') }}
      </p>
      ...
      <template v-if="isEvent">
        ...
        <p class="tips">
          {{ $t('这里以一个站点需要统计用户使用搜索功能频率为例') }}
        </p>
        <p class="tips">
          - {{ $t('首先需要引入上述提供的js脚本并初始化') }}
        </p>
        <p class="tips">
        ...
        <p class="tips">
          - {{ $t('也可以通过') }} <code>sendEvent</code> {{ $t('API主动上报你的自定义事件数据，具体代码如下：') }}
        </p>
        <div class="examples">
        ...
        <p class="tips">
          {{ $t('更多请参考：') }} <a
            :href="GLOBAL.DOC.PA_ANALYSIS_CUSTOM"
            target="_blank"
          > {{ $t('如何接入自定义事件统计') }} </a>
        </p>
      </template>
      <template v-else>
        <p class="tips mb10">
          {{ $t('更多请参考：') }} <a
            :href="GLOBAL.DOC.PA_ANALYSIS_WEB"
            target="_blank"
          > {{ $t('如何接入网站访问统计') }} </a>
        </p>
      </template>
