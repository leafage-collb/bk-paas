  ...
  >
    ...
    >
      ...
      <bk-table-column :label="$t('结果')">
        <template slot-scope="{ row }">
          <div v-if="row.status === 'successful'">
            <span class="g-dot-default successful mr5" />
            {{ $t('成功') }}
          </div>
          <div v-else-if="row.status === 'failed'">
            <span class="g-dot-default failed mr5" />
            {{ $t('失败') }}
          </div>
          <div v-else-if="row.status === 'interrupted'">
            <span class="g-dot-default not_deployed mr5" /> {{ $t('中断') }}
          </div>
          <div v-else-if="row.status === 'pending' && row.operation_type === 'online'">
            <round-loading class="mr5" /> {{ $t('部署中') }}
          </div>
          <div v-else-if="row.status === 'pending' && row.operation_type === 'offline'">
            <span class="g-dot-default not_deployed mr5" /> {{ $t('下架中') }}
          </div>
        </template>
      </bk-table-column>
      <bk-table-column
    ...
    >
      <div
        slot="header"
        class="flex-row align-items-center justify-content-between gap-12"
      >
        <div
          class="text-ellipsis"
          v-bk-overflow-tips
