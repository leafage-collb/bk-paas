    ...
    >
      ...
      >
        <div class="ps-top-card">
          ...
          <p class="desc">
            {{ $t('开启移动端配置后允许通过腾讯企业号') }} -> {{ GLOBAL.HELPER.name }} {{ $t('访问蓝鲸应用') }}
            <a
        ...
        <div
          v-if="mobileConfig.canReleaseToMobile"
          class="content"
        >
          <div class="controller">
            ...
            >
              <div class="header">
                ...
                <div
                  class="action"
                  @click.stop.prevent="handleShowDialog(key)"
                >
                  <bk-switcher
              ...
              >
                <template v-if="mobileConfig[key].is_enabled">
                  <div class="url">
                    {{ $t('移动端访问地址：') }} <span
                      v-bk-tooltips="mobileConfig[key].access_domain || '--'"
                      class="text"
                    >{{ mobileConfig[key].access_domain || '--' }}</span>
                    <i
                      v-bk-tooltips="$t('编辑')"
                      class="paasng-icon paasng-edit2 edit"
                      @click="handleEdit(key, mobileConfig[key].access_domain)"
                    />
                  </div>
                  <div>
                    <wx-qiye-qrcode
      ...
      >
        ...
        >
          ...
          >
            <bk-input
              v-model="dialogConfig.url"
              :placeholder="`${$t('如')}：http://appid.${GLOBAL.WOA_DOMAIN}.com/mobile/`"
            />
          </bk-form-item>
        </bk-form>
        <bk-alert
          class="mt10"
          :title="$t('只支持 woa 域名，请先在微信、企业微信上验证地址是否访问正常')"
        />
      ...
      >
        <p class="mb5">
          {{ $t('请完整输入') }} <code>{{ appCode }}</code> {{ $t('来确认停用移动端配置！') }}
        </p>
        <bk-input
          v-model="disableDialogConfig.appCode"
      ...
      >
        <p
          v-if="disableDialogConfig.env === 'stag'"
          class="tl"
        >
          {{ $t('停用后，在') }} {{ GLOBAL.HELPER.name }} {{ $t('测试企业号中将不能访问该应用') }}
        </p>
        <p
          v-else
          class="tl"
        >
          {{ $t('停用后，在') }} {{ GLOBAL.HELPER.name }} {{ $t('企业号中将不能访问该应用') }}
        </p>
        <template slot="footer">
          <bk-button
            theme="primary"
