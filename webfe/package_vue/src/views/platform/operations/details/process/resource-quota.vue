  ...
  <div class="resource-quota-container card-style">
    ...
    >
      ...
      <div
        ...
        v-else
      >
        <!-- 查看态，兼容情况三：预设方案，传 resources: { plan }，通过 resources.plan 在 planList 中获取对应的limits、requests -->
        <SectionContainer
...
export default {
  ...
  methods: {
    ...
    // 获取资源配额方案名称
    getPlanName(env) {
      // 情况三：预设方案，resources: { plan: '预设方案名称' }
      const envData = this.processData?.env_overlays?.[env];
      if (envData?.resources?.plan) {
        const plan = this.planList.find((item) => item.name === envData.resources.plan);
        return plan?.name || envData.resources.plan || '--';
      }
    ...
    // 获取查看态展示的资源数据（兼容情况三：resources.plan）
    ...
    initFormData() {
      ...
      // 如果 stag 和 prod 的 resources 都为 null，则为 'file'（以描述文件为准），否则为 'custom'（自定义）
      const isStagNull = stag?.resources === null;
      const isProdNull = prod?.resources === null;
      ...
      [
        ...
        // resources 不为 null 且不为 undefined 表示有资源配额数据
        if (data.resources !== null && data.resources !== undefined) {
          // 情况三：预设方案，resources: { plan: '预设方案名称' }
          if (data.resources.plan) {
            this.formData[env].plan_name = data.resources.plan;
            // 从 planList 中获取对应的 limits 和 requests
            const plan = this.planList.find((item) => item.name === data.resources.plan);
            if (plan) {
          ...
          // 情况二：自定义资源配额，resources: { limits: {}, requests: {} }
          else if (data.resources.limits || data.resources.requests) {
    ...
    async updateProcessResources() {
      ...
      ['stag', 'prod'].forEach((env) => {
        envOverlays[env] = {
          resources: this.getResourcesParams(env),
        };
      });
