...
export default {
  ...
  methods: {
    ...
    async getBindingPolicies(serviceId) {
      try {
        const res = await this.$store.dispatch('tenant/getBindingPolicies', {
          serviceId,
        });
        // 数据二次处理
        this.bindingPolicies = res.map((item) => {
          const { allocation_precedence_policies = [], allocation_policy = {} } = item;
          // 对 allocation_precedence_policies 进行排序
          const sortedPolicies = allocation_precedence_policies.sort((a, b) => b.priority - a.priority);
          // 将 allocation_policy 添加到 displayPolicies 数组末尾作为按规则配置的 else 项
          const displayPolicies = [...sortedPolicies, allocation_policy];
          const isConfig = allocation_precedence_policies.length > 0 || Object.keys(allocation_policy)?.length > 0;
          return {
            ...item,
            isConfig,
            displayPolicies,
        ...
        const totalCount = this.bindingPolicies.length;
        const notConfiguredCount = this.bindingPolicies.filter((item) => !item.isConfig).length;
        this.filterList.forEach((v) => {
          if (v.name === 'all') {
            v.count = totalCount;
          } else if (v.name === 'notConfigured') {
            v.count = notConfiguredCount;
          } else {
            v.count = totalCount - notConfiguredCount;
          }
        });
      } catch (e) {
        this.catchErrorHandler(e);
