            ...
            <template v-if="oauth.associated">
              <li
                :class="[
                  'backend-list',
                  'no-impower',
                  oauth.token_list.length > 1 ? 'set-mt' : '',
                  oauth.token_list.length % 2 !== 0 ? 'set-ml' : '',
                ]"
...
export default {
  data() {
    return {
      loading: false,
      oauth2Backends: [],
      githubLogo: require('@static/images/github-logo.svg'),
      giteeLogo: require('@static/images/gitee-logo.svg'),
      titleInfo: {
